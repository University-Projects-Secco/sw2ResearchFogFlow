    ...
    function JSONObject2CtxElement(ob) {
        ...
        if (ob.attributes) {
            for (key in ob.attributes) {
                ...
                contextElement.attributes.push({name: attr.name, type: attr.type, value: attr.value});
            }
        }

        contextElement.domainMetadata = [];
        if (ob.domainMetadata) {
            for (key in ob.domainMetadata) {
                meta = ob.domainMetadata[key];
                contextElement.domainMetadata.push({name: meta.name, type: meta.type, value: meta.value});
            }
        }
