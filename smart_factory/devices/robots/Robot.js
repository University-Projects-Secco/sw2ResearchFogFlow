...
import type {ENTITY,POSITION} from 'utils/FogFlow_Types.js';
import type {ROBOT_CONFIGURATION} from './robot_types';

const log = require('loglevel');

const STATUS_ENUM = Object.freeze({'idle':0,'working':1,'moving':2});
type STATES = $Keys<typeof STATUS_ENUM>;

export default class Robot extends AbstractDevice{

    static nextId = 0;
    static profile: ROBOT_CONFIGURATION;
    position: number[];
    jobProgression = 0;
    status = 0;

    constructor(initialStatus: STATES | number,profile: ROBOT_CONFIGURATION) {
        super();
        Robot.profile = profile;
        if('number' == typeof initialStatus) this.status = initialStatus;
        else  this.status = STATUS_ENUM[initialStatus] || 0; 
